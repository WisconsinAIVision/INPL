...
class WideResNetCB(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, is_remix=False):
        ...
        self.fc_cb = nn.Linear(channels[3], num_classes)
        ...
        self.rot_classifier.bias.data.zero_()
    ...
    def classify_cb(self, x):
        return self.fc_cb(x)

    def classify_wn(self, x, T):
        norm = torch.linalg.norm(self.fc.weight, dim=1, keepdim=True)
        norm = norm ** T
        normed_weight = self.fc.weight / norm
        return F.linear(x, normed_weight, bias=None)

    def classify_rot(self, x):
        return self.rot_classifier(x)


class build_WideResNetCB:
...
if __name__ == '__main__':
    wrn_builder = build_WideResNetCB(1, 10, 2, 0.01, 0.1, 0.5)
